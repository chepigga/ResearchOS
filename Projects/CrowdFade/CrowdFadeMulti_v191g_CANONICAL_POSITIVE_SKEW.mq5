...
#property strict
#property description "CrowdFade v1.91g canonical broker/timeframe parity + positive-skew exits."
...
input bool   InpUseScore      = false;    // v191g parity: broker-derived score weighting OFF
...
input double InpBreakEvenAtATR= 3.00;     // v191g PROFIT LOCK arm: +2.0R with SL=1.5ATR
input double InpBreakEvenLock = 2.25;     // v191g lock +1.5R; no more +0.1R micro-winners
...
input double InpTrailArmATR   = 3.50;     // later arm; at activation 0.5ATR gap locks ~+2R
...
input double InpMaxSpreadATR  = 0.00;     // parity default: no broker-specific signal blocking; >0 optional execution gate
...
input long   InpMagic         = 77192;    // v191g isolated magic
...
input group "=== v1.91g CANONICAL BINANCE PARITY ==="
input int    InpCanonicalMaxAgeSec = 1200; // latest completed canonical M15 may be up to ~15m old
input bool   InpCanonicalStrict    = true; // no broker-candle fallback for signal/ATR/confirmation
...
struct SymState
  {
   ...
   long     confSourceTimeMs;   // frozen Binance crowd timestamp at arm
...
string GVPrefix()
  {
   return StringFormat("CF191G_%I64d_%I64d_", AccountInfoInteger(ACCOUNT_LOGIN), InpMagic);
  }
   ...
   for(int i = 0; i < n && cnt < MAX_SYM; i++)
     {
      ...
      g_sym[cnt].canonOk=false; g_sym[cnt].canonLastFetch=0;
      g_sym[cnt].canonM15OpenMs=0; g_sym[cnt].canonM15CloseMs=0; g_sym[cnt].canonM15Close=0.0; g_sym[cnt].canonAtr14=0.0;
      g_sym[cnt].canonM5OpenMs=0; g_sym[cnt].canonM5CloseMs=0;
      g_sym[cnt].canonM5Open=0.0; g_sym[cnt].canonM5High=0.0; g_sym[cnt].canonM5Low=0.0; g_sym[cnt].canonM5Close=0.0;
      g_sym[cnt].canonErr="";
...
int OnInit()
  {
   ...
   if(InpWriteCsv)
     {
      string fn="CrowdFade_signals_v191g_canonical_rr.csv";
   ...
   if(InpWriteExecutionCsv)
     {
      g_execCsv=FileOpen("CrowdFade_execution_v191g_canonical_rr.csv",FILE_READ|FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_COMMON|FILE_SHARE_READ,';');
   ...
   PrintFormat("INIT_OK CrowdFadeMulti v1.91g CANONICAL_PARITY_POSITIVE_SKEW | symbols=%d | z=%.2f | max exposure=%d | scheduler=%dms",
   ...
   Print("  v1.91g: Binance canonical M5/M15 drives decisions; broker only executes. Positive-skew lock/trail enabled.");
...
//+------------------------------------------------------------------+
//| v1.91g canonical Binance USD-M futures klines                    |
//+------------------------------------------------------------------+
...
bool CanonicalM15(const string symbol,long &openMs,long &closeMs,double &c,double &atr,string &err)
  {
   long ot[],ct[];double oo[],hh[],ll[],cc[];
   ...
   openMs=ot[last];closeMs=ct[last];c=cc[last];
   return true;
  }
...
bool FetchCanonicalOne(const int idx)
  {
   string e="";
   long m5om=0,m5cm=0,m15om=0,m15cm=0;
   double m5o=0,m5h=0,m5l=0,m5c=0,m15c=0,a=0;
   if(!CanonicalM5(g_sym[idx].binance,m5om,m5cm,m5o,m5h,m5l,m5c,e))
     {g_sym[idx].canonOk=false;g_sym[idx].canonErr="M5 "+e;return false;}
   if(!CanonicalM15(g_sym[idx].binance,m15om,m15cm,m15c,a,e))
     {g_sym[idx].canonOk=false;g_sym[idx].canonErr="M15 "+e;return false;}
   ...
   g_sym[idx].canonM15OpenMs=m15om;g_sym[idx].canonM15CloseMs=m15cm;g_sym[idx].canonM15Close=m15c;g_sym[idx].canonAtr14=a;
...
bool IsFeedFresh(const int idx)
  {
   ...
   long nowMs=(long)TimeGMT()*1000;
   long age15=nowMs-g_sym[idx].canonM15CloseMs;
   if(age15<0) age15=0;
   if(age15>(long)InpCanonicalMaxAgeSec*1000) return !InpCanonicalStrict;
   return true;
  }
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
     {
      ...
      if(baseAtr<=0.0) continue;

      // v191g: management trigger is canonical Binance path, not broker chart/ticks.
      // Broker quotes are used only to place/execute the protective stop.
      double refEntry=GVRead(GVPosKey(posId,"REF_ENTRY"),0.0);
      if(refEntry<=0.0 && g_sym[si].canonOk)
        {
         refEntry=g_sym[si].canonM5Close;
         GVWrite(GVPosKey(posId,"REF_ENTRY"),refEntry);
        }
      double mfeAtr=0.0;
      if(refEntry>0.0 && g_sym[si].canonOk)
        {
         double refPeak=GVRead(GVPosKey(posId,"REF_PEAK"),refEntry);
         if(side>0)
           {
            if(g_sym[si].canonM5High>refPeak) refPeak=g_sym[si].canonM5High;
            mfeAtr=(refPeak-refEntry)/baseAtr;
           }
         else
           {
            if(refPeak<=0.0 || g_sym[si].canonM5Low<refPeak) refPeak=g_sym[si].canonM5Low;
            mfeAtr=(refEntry-refPeak)/baseAtr;
           }
         if(mfeAtr<0.0) mfeAtr=0.0;
         GVWrite(GVPosKey(posId,"REF_PEAK"),refPeak);
        }

      // POSITIVE-SKEW PROFIT LOCK:
      // SL risk = 1.5 ATR = 1R.
      // Default arm +3.0 ATR = +2R MFE; lock +2.25 ATR = +1.5R.
      // This intentionally removes the old +0.15ATR (~+0.1R) micro-winner behavior.
      if(InpBreakEvenAtATR>0.0 && InpBreakEvenLock>0.0 && mfeAtr>=InpBreakEvenAtATR)
        {
         ...
         if(better && dist)
           {
            ...
            LogExec("PROFIT_LOCK",s,tk,(side>0)?"BUY":"SELL",0,lvl,0,actual,rc,
                    verified?StringFormat("OK canonMFE=%.3fATR lock=%.3fATR",mfeAtr,InpBreakEvenLock):"FAIL");
            if(verified){c_be++;csl=actual;}
           }
        }

      // Later canonical trailing. Default arm 3.5 ATR and gap 0.5 ATR:
      // at first activation the protected move is ~+3.0 ATR = +2R.
      if(InpTrailOn && InpTrailATR>0.0 && mfeAtr>=InpTrailArmATR)
        {
         double protectedAtr=mfeAtr-InpTrailATR;
         double lvl=op+side*protectedAtr*baseAtr;
         lvl=NormalizePriceTick(s,lvl,(side>0)?-1:+1);
         bool better=(side>0)?(csl<=0.0 || lvl>csl):(csl<=0.0 || lvl<csl);
         ...
         if(better && dist)
           {
            ...
            LogExec("TRAIL",s,tk,(side>0)?"BUY":"SELL",0,lvl,0,actual,rc,
                    verified?StringFormat("OK canonMFE=%.3fATR gap=%.3fATR",mfeAtr,InpTrailATR):"FAIL");
            if(verified){c_trail++;csl=actual;}
           }
        }

      if(InpPartialClose && InpPartialFrac>0.05 && InpPartialFrac<0.95)
        {
               ...
               if(part>=vmin && (vol-part)>=vmin)
                 {
                  ...
                  LogExec("PARTIAL",s,tk,(side>0)?"BUY":"SELL",0,0,0,0,rc,b?"sent":"fail");
      ...
      if(InpExitZ>0.0 && IsFeedFresh(si))
        {
         double z=g_sym[si].z;
         bool against=(side<0)?(z<=-InpExitZ):(z>=InpExitZ);
         if(against)
           {
            bool b=g_trade.PositionClose(tk);uint rc=g_trade.ResultRetcode();
...
void TryEnter(const int idx)
  {
   ...
   // v1.91g: ONE canonical decision clock for every broker and every chart timeframe.
   // Only the latest COMPLETED Binance M5 bar can advance the state machine.
   datetime bt=(datetime)(g_sym[idx].canonM5CloseMs/1000);
   if(bt<=0 || bt==g_sym[idx].lastBar) return;
   g_sym[idx].lastBar=bt;
   c_sig++;

   double barClose=g_sym[idx].canonM5Close;
   double barHigh =g_sym[idx].canonM5High;
   double barLow  =g_sym[idx].canonM5Low;
   double atr     =g_sym[idx].canonAtr14;
   if(barClose<=0.0 || atr<=0.0) return;

   double z=g_sym[idx].z;

   // ========== CANONICAL CONFIRMATION STATE MACHINE ==========
   if(InpUseConfirm)
     {
      // A. Existing thesis: response, adverse excursion and confirm are Binance-native.
      if(g_sym[idx].confActive)
        {
         ...
         int ageSec=(g_sym[idx].confSignalTime>0)?(int)(TimeGMT()-g_sym[idx].confSignalTime):0;

         double fav=0.0,adv=0.0;
         if(side<0)
           {
            fav=(sigPx-barLow)/sigAtr;   // SELL thesis benefits from lower price
            adv=(barHigh-sigPx)/sigAtr;
           }
         else
           {
            fav=(barHigh-sigPx)/sigAtr;  // BUY thesis benefits from higher price
            adv=(sigPx-barLow)/sigAtr;
           }
         if(fav<0.0) fav=0.0;
         if(adv<0.0) adv=0.0;
         if(fav>g_sym[idx].confMaxFavATR) g_sym[idx].confMaxFavATR=fav;
         if(adv>g_sym[idx].confMaxAdvATR) g_sym[idx].confMaxAdvATR=adv;
         ...
         if(g_sym[idx].confBarsLeft<=0 || (InpConfirmMaxAgeMin>0 && ageSec>InpConfirmMaxAgeMin*60))
           {
            ...
            LogExec("CONFIRM_TIMEOUT",sym,0,(side<0)?"SELL":"BUY",0,0,0,0,0,
                    StringFormat("CANON age=%ds fav=%.3f adv=%.3f response=%.3f",ageSec,g_sym[idx].confMaxFavATR,g_sym[idx].confMaxAdvATR,rr));
            return;
           }
         ...
         if(InpConfirmMaxAdverseATR>0.0 && g_sym[idx].confMaxAdvATR>InpConfirmMaxAdverseATR)
           {
            double rr=g_sym[idx].confMaxFavATR/(g_sym[idx].confMaxAdvATR+1e-9);
            g_sym[idx].confActive=false;
            LogExec("CONFIRM_CANCEL_ADVERSE",sym,0,(side<0)?"SELL":"BUY",0,0,0,0,0,
                    StringFormat("CANON age=%ds fav=%.3f adv=%.3f response=%.3f",ageSec,g_sym[idx].confMaxFavATR,g_sym[idx].confMaxAdvATR,rr));
            return;
           }
         ...
         double atrExpansion=(sigAtr>0.0)?(currentAtr/sigAtr):0.0;
         bool sameCrowdSide=(side<0)?(z>0.0):(z<0.0);
         if(!sameCrowdSide)
           {
            g_sym[idx].confActive=false;
            LogExec("CONFIRM_CANCEL_SIDE",sym,0,(side<0)?"SELL":"BUY",0,0,0,0,0,
                    StringFormat("CANON origZ=%+.3f z=%+.3f age=%ds ATRx=%.3f",originalZ,z,ageSec,atrExpansion));
            return;
           }
         ...
         if(InpConfirmMinAbsZ>0.0 && MathAbs(z)<InpConfirmMinAbsZ)
           {
            g_sym[idx].confActive=false;
            LogExec("CONFIRM_CANCEL_WEAK_Z",sym,0,(side<0)?"SELL":"BUY",0,0,0,0,0,
                    StringFormat("CANON origZ=%+.3f z=%+.3f age=%ds",originalZ,z,ageSec));
            return;
           }
         ...
         if(InpConfirmMinResponseRatio>0.0 && responseRatio<InpConfirmMinResponseRatio)
           {
            g_sym[idx].confActive=false;
            LogExec("CONFIRM_CANCEL_NOISE",sym,0,(side<0)?"SELL":"BUY",0,0,0,0,0,
                    StringFormat("CANON origZ=%+.3f z=%+.3f age=%ds fav=%.3f adv=%.3f response=%.3f",
                                 originalZ,z,ageSec,g_sym[idx].confMaxFavATR,g_sym[idx].confMaxAdvATR,responseRatio));
            return;
           }
         ...
         if(contradictsExit)
           {
            g_sym[idx].confActive=false;
            LogExec("CONFIRM_CANCEL_Z",sym,0,(side<0)?"SELL":"BUY",0,0,0,0,0,
                    StringFormat("CANON origZ=%+.3f z=%+.3f age=%ds",originalZ,z,ageSec));
            return;
           }

         string confirmDiag=StringFormat("CANON m5=%I64d source=%I64d origZ=%+.3f z=%+.3f age=%ds sigATR=%.8f curATR=%.8f ATRx=%.3f fav=%.3f adv=%.3f response=%.3f",
                                         (long)bt,g_sym[idx].confSourceTimeMs,originalZ,z,ageSec,sigAtr,currentAtr,atrExpansion,
         ...
         // Pause/day/exposure are evaluated using the same canonical price state on every broker.
         ...
         if(InpMaxSpreadATR>0.0 && (spr<=0.0 || spr>InpMaxSpreadATR*sigAtr))
           {c_spread++;LogExec("EXEC_SPREAD_SKIP",sym,0,(side<0)?"SELL":"BUY",0,0,0,0,0,StringFormat("spread=%.8f canonATR=%.8f",spr,sigAtr));return;}

         double entry=(side<0)?q.bid:q.ask;
         if(!InpConfirmMarket)
           entry=(side<0)?entry-InpConfirmLimitATR*sigAtr:entry+InpConfirmLimitATR*sigAtr;
         ...
         // v191g parity: fixed risk weight. Broker-derived legacy score cannot alter reachability/sizing.
         double wgt=1.0;
         ...
         if(!InpDryRun)
           {
            string sig=StringFormat("CF191G%s%I64d",(side<0)?"S":"B",g_sym[idx].confSourceTimeMs/1000);
            g_trade.SetTypeFillingBySymbol(sym);
            c_sent++;
            bool basic=false;
            if(InpConfirmMarket)
               basic=(side<0)?g_trade.Sell(lot,sym,0.0,stop,0.0,sig):g_trade.Buy(lot,sym,0.0,stop,0.0,sig);
            else
              {
            ...
            if(!ok)
              {
               c_fail++;act="FAIL";
               LogExec("ORDER_SEND",sym,ord,(side<0)?"SELL":"BUY",entry,stop,0,0,rc,"FAIL "+confirmDiag);
              }
            else
              {
               double ap=0.0,asl=0.0;bool verified=VerifyOrderState(ord,entry,stop,ap,asl);
               RegisterEntry(idx,barClose,sigAtr);
               GVWrite(GVOrderKey(ord,"ATR"),sigAtr);
               GVWrite(GVOrderKey(ord,"REF_ENTRY"),barClose);
               GVWrite(GVOrderKey(ord,"SOURCE_MS"),(double)g_sym[idx].confSourceTimeMs);
               GVWrite(GVOrderKey(ord,"CHASE_DONE"),1.0);
         ...
         if(g_csv!=INVALID_HANDLE)
           {
            FileWrite(g_csv,
                      ...
                      DoubleToString(sigAtr,8),DoubleToString(barClose,8),(side<0)?"SELL":"BUY",
      ...
      // B. No active thesis: arm from the canonical completed Binance M5 close.
      if(HasAnyFor(sym)) return;
      int side=0;
      ...
      g_sym[idx].confSignalMid=barClose;
      g_sym[idx].confAtr=atr;
      g_sym[idx].confZ=z;
      g_sym[idx].confMaxFavATR=0.0;
      g_sym[idx].confMaxAdvATR=0.0;
      g_sym[idx].confSignalTime=bt;
      g_sym[idx].confSourceTimeMs=g_sym[idx].sourceTimeMs;
      g_sym[idx].confBarsLeft=InpConfirmMaxBars;

      LogExec("CONFIRM_ARM",sym,0,(side<0)?"SELL":"BUY",0,0,0,0,0,
              StringFormat("CANON m5=%I64d source=%I64d z=%+.3f px=%.8f atr=%.8f",
                           (long)bt,g_sym[idx].confSourceTimeMs,z,barClose,atr));
      return;
     }

   // Legacy no-confirm mode remains available, but the SIGNAL/ATR clock is still canonical.
   int side=0;
   if(z>=InpZThreshold) side=-1;
   else if(z<=-InpZThreshold) side=1;
   if(side==0){c_thr++;return;}
   ...
   double spr=q.ask-q.bid;
   if(InpMaxSpreadATR>0.0 && (spr<=0.0 || spr>InpMaxSpreadATR*atr)){c_spread++;return;}
   double ref=(side<0)?q.bid:q.ask;
   ...
   if(!InpDryRun)
     {
      datetime exp=TimeCurrent()+(datetime)(InpOrderValidHrs*3600);
      string sig=StringFormat("CF191G%s%I64d",(side<0)?"S":"B",g_sym[idx].sourceTimeMs/1000);
      g_trade.SetTypeFillingBySymbol(sym);c_sent++;
      bool basic=(side<0)?g_trade.SellLimit(lot,entry,sym,stop,0.0,ORDER_TIME_SPECIFIED,exp,sig)
                         :g_trade.BuyLimit(lot,entry,sym,stop,0.0,ORDER_TIME_SPECIFIED,exp,sig);
      rc=g_trade.ResultRetcode();ord=g_trade.ResultOrder();
      bool ok=basic && RetcodeAccepted(rc) && ord>0;
      if(!ok){c_fail++;act="FAIL";LogExec("ORDER_SEND",sym,ord,(side<0)?"SELL":"BUY",entry,stop,0,0,rc,"FAIL");}
      else
        {
         double ap=0.0,asl=0.0;bool verified=VerifyOrderState(ord,entry,stop,ap,asl);
         RegisterEntry(idx,barClose,atr);
         GVWrite(GVOrderKey(ord,"ATR"),atr);
         GVWrite(GVOrderKey(ord,"REF_ENTRY"),barClose);
         GVWrite(GVOrderKey(ord,"SOURCE_MS"),(double)g_sym[idx].sourceTimeMs);
         GVWrite(GVOrderKey(ord,"CHASE_DONE"),1.0);
      ...
      if(g_sym[idx].lastAttempt==0 || now-g_sym[idx].lastAttempt>=InpRefreshSec)
        {
         ...
         if(InpVerbose && (!crowdOk || !canonOk))
            PrintFormat("FETCH_PARITY %s crowd=%s canon=%s err=%s canonErr=%s",
                        g_sym[idx].broker,crowdOk?"OK":"FAIL",canonOk?"OK":"FAIL",g_sym[idx].err,g_sym[idx].canonErr);
         return;
        }
     }
  }
...
void OnTradeTransaction(const MqlTradeTransaction &trans,
                        ...
                        const MqlTradeResult &result)
  {
   ...
   int si=SymIndex(sym);
   if(atr<=0.0 && si>=0 && g_sym[si].canonOk) atr=g_sym[si].canonAtr14;
   if(refEntry<=0.0 && si>=0 && g_sym[si].canonOk) refEntry=g_sym[si].canonM5Close;
   ...
   LogExec("FILL",sym,order,"",px,0,px,0,(uint)result.retcode,
           StringFormat("pos=%I64u canonATR=%.8f refEntry=%.8f",posId,atr,refEntry));
  }
//+------------------------------------------------------------------+
