...
int OnInit()
  {
   ...
   if(InpShadowRetraceEnabled && (InpShadowRetraceATR<0.0 || InpShadowRetraceATR>=InpConfirmLimitATR))
     { Print("ERROR: shadow retrace must be >=0 and strictly smaller than real CORE retrace"); return INIT_PARAMETERS_INCORRECT; }
   ...
   if(InpShadowRetraceEnabled && InpShadowWriteCsv)
     {
      g_shadowCsv=FileOpen("CrowdFade_shadow_v200b_retrace040.csv",FILE_READ|FILE_WRITE|FILE_CSV|FILE_ANSI|FILE_COMMON|FILE_SHARE_READ,';');
      if(g_shadowCsv==INVALID_HANDLE) PrintFormat("SHADOW CSV not opened, error=%d",GetLastError());
      else
        {
         bool empty=(FileSize(g_shadowCsv)==0);
         FileSeek(g_shadowCsv,0,SEEK_END);
         if(empty) FileWrite(g_shadowCsv,"server","utc","event","symbol","source_ms","real_order","side","real_retrace_atr","shadow_retrace_atr","broker_ref","signal_atr","shadow_entry","shadow_sl","shadow_tp","shadow_lot","risk_cash","real_filled","additional_finalized","bid","ask","exit_reason","exit_px","gross_R","net_R","net_cash","note");
         FileFlush(g_shadowCsv);
        }
     }

   EventSetMillisecondTimer(InpExecTimerMs);
   PrintFormat("INIT_OK CrowdFadeMulti v2.00b ACCOUNT_MODE_NEUTRAL | symbols=%d | zLong=%.2f | zShort=%.2f | max exposure=%d | scheduler=%dms",
   ...
   if(InpShadowRetraceEnabled)
      PrintFormat("  SHADOW: retrace %.2f ATR | NO REAL ORDERS | same TTL=%dmin | SL %.2f ATR | TP %.2f ATR | H%d",
                  InpShadowRetraceATR,InpOrderValidMin,InpStopATR,InpTakeProfitATR,InpHoldHours);
   ...
   Print("  v2.00a frozen core unchanged; retrace0.40 SHADOW is telemetry-only and cannot trade.");
...
void OnDeinit(const int reason)
  {
   ...
   if(g_shadowCsv != INVALID_HANDLE) { FileFlush(g_shadowCsv); FileClose(g_shadowCsv); }
   Comment("");
   Print("=== КАСКАД ===");
   ...
   PrintFormat("  SHADOW R%.2f: armed=%d fills=%d additional=%d same=%d nofill=%d exits=%d net=%.3fR / %.2f cash | ADDITIONAL closed=%.3fR / %.2f cash",
               InpShadowRetraceATR,c_shadow_arm,c_shadow_fill,c_shadow_additional,c_shadow_same,c_shadow_nofill,c_shadow_exit,c_shadow_net_r,c_shadow_net_cash,c_shadow_additional_r,c_shadow_additional_cash);
      ...
      if(!InpDryRun)
        {
         ...
         if(!ok)
         ...
         else
           {
            ...
            ArmShadowRetrace(idx,g_sym[idx].confSourceTimeMs,ord,side,brokerRef,sigAtr,lot,riskMult);
...
void DrawPanel()
  {
   ...
   string t=StringFormat("CrowdFade v2.00b  zL=%.2f zS=%.2f  exposure %d/%d%s\n",
...
void OnTimer()
  {
   ...
   // Shadow telemetry is observational and continues even if the real EA hits a risk halt.
   ManageShadowRetrace();
      ...
      if(posId>0)
        {
         ...
         // Shadow 0.40 classification: remember whether the actual 0.60 CORE order filled within its TTL.
         if(!isFastDeal) MarkShadowRealFill(order);
