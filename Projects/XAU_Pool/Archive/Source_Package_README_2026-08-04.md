...
## Артефакти (Projects/AK47/Artifacts/)

```
weights_schedule_XAU_POOL_v001.pkl   48 WF-вікон, 36 фіч, коефіцієнти,
                                     intercept, mean, scale на кожне вікно
pool_excess.parquet                  266 297 кандидатів з мітками R і excess
baseline.parquet                     дрейф по (місяць x напрямок x ТФ)
permutation_37_shuffles.jsonl        сирі результати GATE-4
MANIFEST_SHA256.csv                  контрольні суми всіх файлів пакета
```

**Модель збережена** — на відміну від блокера B5 у FXArena, де ваги
було втрачено і реконструкція провалила AC3/AC4.

Сирі дані M1 (155 MB) у пакет НЕ входять — вони вже в релізі `ak47`.
...
Шлях до даних задається змінною середовища:

```bash
export XAU_DATA=/шлях/до/даних     # за замовчуванням — каталог скрипта
```

Скрипти очікують `xau_m1.parquet` у `$XAU_DATA`. Проміжні артефакти
створюються автоматично; готові лежать у `Artifacts/`.

**Відтворюваність:** усі генератори випадкових чисел мають фіксовані
seed. У `gate4_perm.py` кожна ітерація має власний seed `2026+it`,
тому відновлення з контрольної точки не порушує відтворюваності
(у першій версії був дефект — стан RNG не відновлювався).
