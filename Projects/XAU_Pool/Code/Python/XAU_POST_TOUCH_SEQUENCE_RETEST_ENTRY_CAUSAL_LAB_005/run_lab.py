...
def load_prices(path:Path)->pd.DataFrame:
    ...
    return df[df.time<pd.Timestamp('2025-07-01')].copy().reset_index(drop=True)
...
def load_events(path:Path)->pd.DataFrame:
    e=pd.read_csv(path,compression='gzip'); e['time']=pd.to_datetime(e.time); e=e[e.time<pd.Timestamp('2025-07-01')].copy()
...
def build_signals(e:pd.DataFrame,clock:int)->pd.DataFrame:
    scol=f'final_side_{clock}m_atr'; z=e[np.isfinite(e[scol])].copy(); z['s']=z[scol].astype(float); z=z[(z.s>=SIGNAL_THR)|(z.s<=-SIGNAL_THR)].copy()
    z['branch']=np.where(z.s>=SIGNAL_THR,'BACK','THROUGH'); z['dir']=np.where(z.branch.eq('BACK'),z.arrival_side,-z.arrival_side).astype(int)
    z['decision_i']=z.i.astype(int)+clock; z['decision_time']=z.time+pd.to_timedelta(clock,unit='m'); z['signal_correct']=np.where(z.branch.eq('BACK'),z.label_0p5.eq('REJECTION'),z.label_0p5.eq('ACCEPTANCE')); z['level_rank']=z.level.map(LEVEL_RANK).astype(int)
    return z

def dedupe_decisions(z:pd.DataFrame)->pd.DataFrame:
    x=z.copy(); x['abs_s']=x.s.abs(); x=x.sort_values(['decision_time','dir','abs_s','level_rank'],ascending=[True,True,False,True]).drop_duplicates(['decision_time','dir'],keep='first')
    counts=x.groupby('decision_time').dir.nunique(); conflict=set(counts[counts>1].index)
...
def sim_trade(entry_i,d,entry,risk,target,times_m,bh,bl,bc,ah,al,ac):
    ...
    return max(-1.0,min(target,r)),last,3
...
def enrich_retests(z:pd.DataFrame,df:pd.DataFrame,clock:int)->pd.DataFrame:
    ...
    for r in z.itertuples(index=False):
        ...
        conf_i,entry_i=find_retest(di,d,atr,lc,times_m,bh,bl,bc,mid,hi,lo); filled=entry_i>=0; market_entry=ac[di] if d>0 else bc[di]
...
def tkey(target:float)->str:return str(target).replace('.','p')

def simulate_all(x:pd.DataFrame,df:pd.DataFrame,target:float)->pd.DataFrame:
    y=x.copy(); times_m=(df.time.astype('int64')//60_000_000_000).to_numpy(np.int64); bh=df.high.to_numpy(float); bl=df.low.to_numpy(float); bc=df.close.to_numpy(float); ah=df.ask_high.to_numpy(float); al=df.ask_low.to_numpy(float); ac=df.ask_close.to_numpy(float)
    ...
    k=tkey(target); y[f'gross_R_{k}']=gross; y[f'net_R_{k}']=net; y[f'stress05_R_{k}']=stress05; y[f'stress10_R_{k}']=stress10; y[f'exit_i_{k}']=exit_i; y[f'outcome_{k}']=outcome; y[f'market_gross_R_{k}']=market_gross; y[f'market_net_R_{k}']=market_net; y[f'uplift_R_{k}']=y[f'net_R_{k}']-y[f'market_net_R_{k}']; y[f'exit_time_{k}']=[df.at[ei,'time'] if ei>=0 else pd.NaT for ei in exit_i]
    return y

def build_serial(x:pd.DataFrame,target:float)->tuple[pd.DataFrame,pd.DataFrame]:
    rows=[]; lifecycles=[]; busy_until=pd.Timestamp.min; k=tkey(target)
...
def maxdd(vals):
    ...
    c=np.cumsum(v); peak=np.maximum.accumulate(np.r_[0,c]); return float((peak[1:]-c).max())
...
def stats(tr:pd.DataFrame,target:float)->dict:
    ...
    return {'n':int(len(v)),'trades_per_week':float(len(v)/weeks) if weeks else np.nan,'ev':float(v.mean()),'pf':pf(v),'positive_rate':float((v>0).mean()),'total_R':float(v.sum()),'gross_ev':float(tr[f'gross_R_{k}'].mean()),'tp_rate':float((tr[f'outcome_{k}']=='TP').mean()),'max_dd_R':maxdd(v),'worst_day_R':float(daily.min()) if len(daily) else np.nan,'max_consec_losses':max_consec_loss(v),'stress05_ev':float(tr[f'stress05_R_{k}'].mean()),'stress10_ev':float(tr[f'stress10_R_{k}'].mean()),'buy_ev':float(tr.loc[tr.dir==1,col].mean()),'sell_ev':float(tr.loc[tr.dir==-1,col].mean()),'back_ev':float(tr.loc[tr.branch=='BACK',col].mean()),'through_ev':float(tr.loc[tr.branch=='THROUGH',col].mean()),'mean_entry_improvement_atr':float(tr.entry_improvement_atr.mean()),'median_entry_improvement_atr':float(tr.entry_improvement_atr.median()),'median_wait_entry_min':float(tr.wait_entry_min.median())}
...
def main():
    ...
    pd.DataFrame(summaries).to_csv(a.outdir/'summary.csv',index=False)
    primary=serial_store[(3,1.5,'CONFIRMATION')]; disc=serial_store[(3,1.5,'DISCOVERY')]; prim_life=lifecycle_store[(3,1.5,'CONFIRMATION')]; st=stats(primary,1.5); sd=stats(disc,1.5); st2=stats(serial_store[(3,2.0,'CONFIRMATION')],2.0); w=bootstrap_weekly(primary,'net_R_1p5'); up=bootstrap_weekly(primary,'uplift_R_1p5',20260823)
    ...
    audit={'lab':LAB,'version':VERSION,'canonical_sha256':h,'raw_preholdout_rows':int(len(df)),'lab002_events':int(len(e)),'ask_execution_present':bool(ask_ok),'lineage_ok':bool(lineage),'holdout_opened':False}; verdict={'status':status,'gates':gates,'primary_confirmation':st,'primary_discovery':sd,'confirmation_2R':st2,'weekly_ev_bootstrap':w,'paired_retest_vs_market_bootstrap':up,'fill_confirmation':fill_conf,'fill_discovery':fill_disc,'holdout_opened':False}
