...
def manage(A,entry_i,entry,side,a,stop_atr):
    ts,bo,bh,bl,bc,ao,ah,al,ac,ft,z,*_=A; sl=entry-side*stop_atr*a; peak=entry; mfe=0.; armed=False; et=int(ts[entry_i]); end=min(len(ts)-1,int(np.searchsorted(ts,et+HOLD,'left'))); last5=int(ts[entry_i]//300)
...
def main():
    A=prepare(); out={'data':{'broker':'FTMO-Demo','period':'2026-05-01..2026-08-07','execution':'native bid/ask 1-second aggregation from MT5 ticks','commission':'not included','m5_exit':'first broker second after each M5 boundary'}}
    res={}
    for c in CONFIRMS:
        for s in STOPS:
