    ...
    while k<len(dt5)-3:
        ...
        if conf:
            ...
            if gate:
                ...
                if np.isfinite(ret60) and np.isfinite(eff60) and ret60>1.0 and eff60<0.25:
...
def main():
    ...
    for period,p in periods.items():
        ...
        rows += [dict(period=period,variant='CONTROL',**bm,positive_periods=bpos,periods_total=btot,skips=0),
                 dict(period=period,variant='SKIP_RET60_GT_1ATR_AND_EFF60_LT_0P25',**nm,positive_periods=npos,periods_total=ntot,skips=nskip)]
        detail[period]={'control':bm,'new':nm,'skip_count':int(nskip),'control_periods':bper,'new_periods':nper,'PASS':passed}
    pd.DataFrame(rows).to_csv(OUT/'comparison.csv',index=False)
    result={'lab':'CROWDFADE_CF191G_RET60_EFF60_EXHAUSTION_SKIP_LAB_055C','threshold_ret60_atr':1.0,'threshold_eff60':0.25,'detail':detail,'PASS_BOTH':overall,
    ...
    lines=['# LAB055C — RET60 > +1.0 ATR AND EFF60 < 0.25 SKIP','',pd.DataFrame(rows).to_markdown(index=False),'',json.dumps(result,indent=2,default=float)]
