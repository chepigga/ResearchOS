...
# UTC M15 signal-bar-start windows:
# A = 04:00-07:59 (hours 4,5,6,7)
# B = 18:00-20:59 (hours 18,19,20)
MODES=[
    ('BASELINE',0),
    ('VETO_04_07',1),
    ('VETO_18_20',2),
...
def sim(ts,O,H,L,C,dt,CL,Z,A,Y,DAY,HOUR,mode):
    ...
    while k<len(dt)-3:
        ...
        if is_vetoed(HOUR[k],mode):k+=1;continue
...
def rec(name,mode,r):
    ...
    return {
        'mode':name,'mode_id':int(mode),'N':int(n),'trades_per_month':float(n/60.0),
...
def main():
    ...
    out={
        'lab':'CROWDFADE_COARSE_TIME_WINDOW_VETO_LAB_018',
        'frozen_signal':{'ZLong':ZTH,'ZShort':ZTH},
        'frozen_execution':{
            'TF':'M15','ConfirmATR':CONF,'ConfirmTTL_min':CONF_TTL_MIN,'RetraceATR':RETRACE,
