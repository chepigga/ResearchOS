...
ROOT=Path('labs/CROWDFADE_SIDE_THRESHOLD_ASYMMETRY_LAB_016')
...
LONG_GRID=[2.25,2.50,2.75]
SHORT_GRID=[2.50,2.75,3.00]
...
def prep():
    ...
    mu=r.rolling(72,min_periods=72).mean()
    sd=r.rolling(72,min_periods=72).std(ddof=0)
    ...
    ok=(idx2>=0)&(idx2<len(ts))
    starts=starts[ok]; idx2=idx2[ok]
    dt=starts+900; cl=C[idx2]
    fi=np.searchsorted(ft,dt,'left')-1
    ai=np.searchsorted(av,dt,'right')-1
...
def stats(a):
    ...
    eq=np.cumsum(a); pk=np.maximum.accumulate(np.r_[0.,eq])
    dd=float((pk[1:]-eq).max())
    p=a[a>0].sum(); n=abs(a[a<0].sum())
    return {
        'N':int(len(a)),
        'trades_per_month':float(len(a)/6.0),
        'WR':float((a>0).mean()),
        'EV':float(a.mean()),
        'SumR':float(a.sum()),
        'PF':float(p/n) if n else 99.,
        'MaxDD_R':dd,
        'R_DD':float(a.sum()/dd) if dd else 99.
    }

def sim(arr,zlong,zshort):
    ts,O,H,L,C,dt,cl,zd,ad=arr
    ...
    while k<len(dt)-2:
        ...
        if dc>=MAXDAY:
            k+=1; continue
        z=float(zd[k])
        side=-1 if z>=zshort else (1 if z<=-zlong else 0)
        if side==0:
            k+=1; continue
        sig=float(cl[k]); a=float(ad[k])
        # Frozen inherited LAB014/LAB015 anti-repeat gate.
        if last is not None and abs(sig-last)<la:
            k+=1; continue
        ...
        ps=np.searchsorted(ts,int(dt[ci])+1)
        pe=np.searchsorted(ts,int(dt[ci])+PTTL,'right')
        ...
        for q in range(ei+1,xe+1):
            ...
            if sh:
                xp=sl; xi=q; reason='SL'; break
            if th:
                xp=tp; xi=q; reason='TP'; break
        ...
        dc+=1; last=entry; la=a
        k=np.searchsorted(dt,int(ts[xi])+1)
    return pd.DataFrame(rows)

def summarize(d):
    if d.empty:
        return {'all':{'N':0},'positive_months':0,'monthly':{},'by_side':{}}
    m=pd.to_datetime(d.entry_ts,unit='s',utc=True).dt.to_period('M').astype(str)
    months=sorted(m.unique())
    return {
        'all':stats(d.R),
        'exit_mix':d.reason.value_counts().to_dict(),
        'positive_months':int(sum(stats(d.loc[m==x,'R'])['SumR']>0 for x in months)),
        'monthly':{x:stats(d.loc[m==x,'R']) for x in months},
        'by_side':{
            side:stats(d.loc[d.side==side,'R'])
...
def main():
    ...
    rows=[]
    for zl in LONG_GRID:
        for zs in SHORT_GRID:
            d=sim(arr,zl,zs)
            s=summarize(d)
            rows.append({'ZLong':float(zl),'ZShort':float(zs),**s})
    baseline=next(x for x in rows if x['ZLong']==2.5 and x['ZShort']==2.5)
    for x in rows:
        a=x['all']; b=baseline['all']
        x['vs_symmetric_2p5']={
            'dEV':float(a.get('EV',0.)-b.get('EV',0.)),
            'dPF':float(a.get('PF',0.)-b.get('PF',0.)),
            'dMaxDD_R':float(a.get('MaxDD_R',0.)-b.get('MaxDD_R',0.)),
            'dSumR':float(a.get('SumR',0.)-b.get('SumR',0.)),
            'dN':int(a.get('N',0)-b.get('N',0))
        }
    out={
        'lab':'CROWDFADE_SIDE_THRESHOLD_ASYMMETRY_LAB_016',
        'period':'2026 seconds forward-shadow/stress',
        'frozen_execution':{
            'TF':'M15','ConfirmATR':CONF,'ConfirmTTL_sec':CONF_TTL,
            'RetraceATR':RETRACE,'LimitOnly':True,'LimitTTL_sec':PTTL,
            'SL_ATR':SL,'TP_ATR':TP,'Hold_sec':HOLD,'Cost_bps':COST,
            'MaxTradesDay':MAXDAY,'InheritedAntiRepeatATR':1.0
        },
        'grid':{'LONG':LONG_GRID,'SHORT':SHORT_GRID},
        'baseline':{'ZLong':2.5,'ZShort':2.5},
        'results':rows,
