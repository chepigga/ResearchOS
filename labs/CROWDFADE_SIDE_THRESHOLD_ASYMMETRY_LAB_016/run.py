...
# LAB016: freeze LAB014/LAB015 execution and vary ONLY directional Z thresholds.
# Important parity detail inherited from LAB014/LAB015: 1 ATR anti-repeat/pause gate stays ON.
...
def prep():
    ...
    q['day']=(q.time.dt.year*10000+q.time.dt.month*100+q.time.dt.day).astype('int64')
    return p,q

@njit(cache=True)
def sim(ts,O,H,L,C,dt,CL,Z,A,Y,D,zlong,zshort):
...
def rec(zlong,zshort,r):
    ...
    sides={}
    for si,name in enumerate(['LONG','SHORT']):
        nn=int(sc[si])
        sides[name]={
            'N':nn,
    ...
    return {
        'ZLong':float(zlong),'ZShort':float(zshort),
...
def main():
    ...
    arr=[
        ...
        q.atr.to_numpy(float),q.yr.to_numpy(np.int64),q.day.to_numpy(np.int64)
    ]
    sim(*[x[:1000] for x in arr],2.5,2.5)
    rows=[]
    for zl in LONG_GRID:
        for zs in SHORT_GRID:
            rows.append(rec(zl,zs,sim(*arr,zl,zs)))
    ...
    out={
        ...
        'rank_EV':sorted(rows,key=lambda x:x['agg']['EV'],reverse=True),
        'rank_RDD':sorted(rows,key=lambda x:x['agg']['R_DD'],reverse=True)
    }
