...
def main():
    ...
    for period,p in periods.items():
        ...
        rows += [dict(period=period,variant='CONTROL',**bm,positive_periods=bpos,periods_total=btot,skips=0),
                 dict(period=period,variant='SKIP_CONFIRM_AGE_GT_15M',**nm,positive_periods=npos,periods_total=ntot,skips=nskip)]
        detail[period]={'control':bm,'new':nm,'skip_count':int(nskip),'control_periods':bper,'new_periods':nper,'PASS':passed}
    pd.DataFrame(rows).to_csv(OUT/'comparison.csv',index=False)
    result={'lab':'CROWDFADE_CF191G_CONFIRM_AGE_SKIP_LAB_055A','threshold_min':15,'detail':detail,'PASS_BOTH':overall,
    ...
    lines=['# LAB055A — CONFIRM AGE >15M SKIP','',pd.DataFrame(rows).to_markdown(index=False),'',json.dumps(result,indent=2,default=float)]
