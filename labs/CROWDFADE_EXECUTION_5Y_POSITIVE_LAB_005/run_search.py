...
import zipfile, json, math, itertools
import numpy as np
import pandas as pd
...
BE_TRAIL=[
 ...
 ('be05',0.50,0.15,None,None),
 ('be075',0.75,0.15,None,None),
 ('be10',1.00,0.25,None,None),
 ('be05_tr20',0.50,0.15,2.00,0.50),
 ('be05_tr25',0.50,0.15,2.50,0.50),
 ('be075_tr20',0.75,0.15,2.00,0.50),
]
...
def load_price():
    ...
    tc=pick(raw,['open_time','timestamp','time','date','datetime'])
    oc=pick(raw,['open']); hc=pick(raw,['high']); lc=pick(raw,['low']); cc=pick(raw,['close'])
    t=raw[tc]
    if np.issubdtype(t.dtype,np.number):
        x=pd.to_numeric(t,errors='coerce')
        unit='ms' if x.dropna().median()>1e11 else 's'
        dt=pd.to_datetime(x,unit=unit,utc=True,errors='coerce')
    else: dt=pd.to_datetime(t,utc=True,errors='coerce')
    ...
    d=d[(d.time>='2021-01-01')&(d.time<'2026-09-01')].reset_index(drop=True)
    return d
...
def load_flow():
    ...
    dt=pd.to_datetime(raw[tc],utc=True,errors='coerce')
    r=pd.to_numeric(raw[rc],errors='coerce')
    f=pd.DataFrame({'time':dt,'ratio':r}).dropna().sort_values('time').drop_duplicates('time')
...
def prepare():
    ...
    # M15 ATR14, only completed bars are made available to M5 rows.
    q=p.set_index('time').resample('15min',label='left',closed='left').agg(o=('o','first'),h=('h','max'),l=('l','min'),c=('c','last')).dropna()
    pc=q.c.shift(1); tr=np.maximum(q.h-q.l,np.maximum((q.h-pc).abs(),(q.l-pc).abs()))
    q['atr']=tr.rolling(14,min_periods=14).mean(); q['avail_time']=q.index+pd.Timedelta(minutes=15)
    ...
    p=p.dropna(subset=['atr','z']).reset_index(drop=True)
    p['year']=p.time.dt.year; p['day']=p.time.dt.floor('D')
    return p

def annual_stats(trades):
    out={}
    for y in YEARS:
        a=[x['R'] for x in trades if x['year']==y]
        arr=np.asarray(a,float)
        if not len(arr):out[str(y)]={'N':0,'SumR':0.0,'EV':0.0,'PF':0.0,'MaxDD':0.0};continue
        eq=np.cumsum(arr); pk=np.maximum.accumulate(np.r_[0.,eq]); dd=float((pk[1:]-eq).max())
        pos=arr[arr>0].sum(); neg=abs(arr[arr<0].sum()); pf=float(pos/neg) if neg>0 else 99.0
...
def simulate(df, confirm, ttl_h, mode, stop_atr, hold_h, pause, exit_policy=('none',None,None,None,None)):
    name,be_at,be_lock,tr_arm,tr_dist=exit_policy
    ...
    while i<n-2:
        t=times[i]
        if t<next_time: i+=1; continue
        day=D[i]; cnt=day_count.get(day,0)
        if cnt>=MAX_TRADES_DAY: i+=1; continue
        ...
        sig_px=O[i]; atr=A[i]; level=sig_px+side*confirm*atr
        bars=max(1,int(ttl_h*12)); end=min(n-2,i+bars)
        ci=None
        ...
        if mode=='touch': ei=ci; entry=level
            ...
            retr=0.25 if mode=='pullback_025' else 0.50
            target=level-side*retr*atr
            pe=min(n-2,ci+12)
            ei=None
            for j in range(ci,pe+1):
        ...
        sl=entry-side*stop_atr*atr; maxbars=max(1,int(hold_h*12)); xe=min(n-1,ei+maxbars)
        peak=entry; armed=False; xp=C[xe]; exit_i=xe; reason='time'
        for j in range(ei+1,xe+1):
            # Conservative intrabar ordering: stop before any management update.
            ...
            peak=max(peak,px) if side>0 else min(peak,px)
            mfe=side*(peak-entry)/atr
            if tr_arm is not None and mfe>=tr_arm:
                ns=peak-side*tr_dist*atr
                if (side>0 and ns>sl) or (side<0 and ns<sl): sl=ns; armed=True
            # opposite crowd state exit, frozen threshold
            if (side<0 and Z[j]<=-EXIT_Z) or (side>0 and Z[j]>=EXIT_Z): xp=px; exit_i=j; reason='zexit'; break
        gross=side*(xp-entry)/(stop_atr*atr)
        cost=(COST_RT_BPS/10000.0)*entry/(stop_atr*atr)
        R=gross-cost
        trades.append({'year':int(Y[ei]),'R':float(R),'entry_i':int(ei),'exit_i':int(exit_i),'reason':reason})
        day_count[D[ei]]=day_count.get(D[ei],0)+1
        last_entry=entry; last_atr=atr
        if pause=='time3h': next_time=times[exit_i]+np.timedelta64(3,'h')
        else: next_time=times[exit_i]+np.timedelta64(5,'m')
        i=exit_i+1
    return trades

def score_candidate(yearly,agg):
    vals=[yearly[str(y)]['SumR'] for y in YEARS]
    miny=min(vals); med=float(np.median(vals));
    # stability first: minimum year, then median year, then aggregate / DD
    return (miny,med,agg['SumR']/max(agg['MaxDD'],1e-9),agg['EV'])
...
def main():
    ...
    for confirm,ttl,mode,stop,hold,pause in itertools.product(CONFIRMS,TTLS,ENTRY_MODES,STOPS,HOLDS,PAUSES):
        tr=simulate(df,confirm,ttl,mode,stop,hold,pause,BE_TRAIL[0]); ann=annual_stats(tr); agg=aggregate(tr)
        pos=sum(ann[str(y)]['SumR']>0 for y in YEARS)
        rec={'confirm':confirm,'ttl_h':ttl,'mode':mode,'stop':stop,'hold_h':hold,'pause':pause,'exit':'none','positive_years':pos,'annual':ann,'agg':agg,'score':score_candidate(ann,agg)}
        coarse.append(rec)
        if pos==5 and agg['N']>=150: survivors.append(rec)
        k+=1
    coarse.sort(key=lambda x:(x['positive_years'],x['score']),reverse=True)
    (OUT/'coarse_top100.json').write_text(json.dumps(coarse[:100],indent=2))
    print('COARSE tested',k,'five-year survivors',len(survivors))
    # If no perfect survivor, expand exits on top 60 near-survivors; else top 40 perfect survivors.
    seeds=(survivors if survivors else coarse[:60])[:60]
    refined=[]
    for s in seeds:
        for ep in BE_TRAIL:
            ...
            r={'confirm':s['confirm'],'ttl_h':s['ttl_h'],'mode':s['mode'],'stop':s['stop'],'hold_h':s['hold_h'],'pause':s['pause'],'exit':ep[0],'positive_years':pos,'annual':ann,'agg':agg,'score':score_candidate(ann,agg)}
            refined.append(r)
    refined.sort(key=lambda x:(x['positive_years'],x['score']),reverse=True)
    perfect=[r for r in refined if r['positive_years']==5 and r['agg']['N']>=150]
    out={'frozen_signal':{'ZThreshold':ZTH,'ExitZ':EXIT_Z},'cost_rt_bps':COST_RT_BPS,'years':YEARS,'coarse_tested':k,'coarse_5y_positive':len(survivors),'refined_tested':len(refined),'refined_5y_positive':len(perfect),'best':refined[0] if refined else coarse[0],'perfect_top20':perfect[:20],'near_top20':[r for r in refined if r['positive_years']<5][:20]}
