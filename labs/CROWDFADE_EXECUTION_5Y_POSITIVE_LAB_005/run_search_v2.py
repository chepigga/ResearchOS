...
def read_klines():
    ...
    for zp in sorted(KDIR.glob('BTCUSDT-5m-*.zip')):
        ...
        x=pd.DataFrame({'ms':pd.to_numeric(r.iloc[:,0],errors='coerce'),'o':pd.to_numeric(r.iloc[:,1],errors='coerce'),'h':pd.to_numeric(r.iloc[:,2],errors='coerce'),'l':pd.to_numeric(r.iloc[:,3],errors='coerce'),'c':pd.to_numeric(r.iloc[:,4],errors='coerce')}).dropna();parts.append(x)
...
def load_flow():
    ...
    raw=raw[pd.to_numeric(raw.sum_open_interest,errors='coerce')>0].copy();raw['time']=pd.to_datetime(raw.create_time,utc=True,errors='coerce').astype('datetime64[ns, UTC]');raw['ratio']=pd.to_numeric(raw.count_long_short_ratio,errors='coerce');raw=raw.dropna(subset=['time','ratio']).sort_values('time').drop_duplicates('time')
...
def prepare():
    ...
    q=p.set_index('time').resample('15min',label='left',closed='left').agg(o=('o','first'),h=('h','max'),l=('l','min'),c=('c','last')).dropna();pc=q.c.shift(1);tr=np.maximum(q.h-q.l,np.maximum((q.h-pc).abs(),(q.l-pc).abs()));q['atr']=tr.rolling(14,min_periods=14).mean();q['avail']=(q.index+pd.Timedelta(minutes=15)).astype('datetime64[ns, UTC]')
    ...
    p=p[(p.time>='2021-01-01')&(p.time<'2026-01-01')].copy();p['ts']=(p.time.astype('int64')//10**9).astype('int64');p['year']=p.time.dt.year.astype('int64');p['day']=(p.time.dt.year*10000+p.time.dt.month*100+p.time.dt.day).astype('int64');return p

@njit(cache=True)
def run_sim(ts,O,H,L,C,Z,A,Y,D,confirm,ttl_h,mode,stop_atr,hold_h,pause,exit_kind,be_at,be_lock,tr_arm,tr_dist):
    sums=np.zeros(5);counts=np.zeros(5,np.int64);n=0;wins=0;pos_sum=0.0;neg_sum=0.0;eq=0.0;peak_eq=0.0;maxdd=0.0;i=0;next_ts=0;last_entry=0.0;last_atr=0.0;has_last=False;cur_day=-1;day_count=0;N=len(ts)
    while i<N-3:
        ...
        R=side*(xp-entry)/(stop_atr*atr)-(COST_BPS/10000.0)*entry/(stop_atr*atr);yi=Y[ei]-2021
    ...
    return sums,counts,n,eq,(eq/n if n>0 else -999.0),(pos_sum/neg_sum if neg_sum>0 else 99.0),maxdd,(wins/n if n>0 else 0.0)
...
def main():
    p=prepare();print('prepared',len(p),p.time.min(),p.time.max());ts=p.ts.to_numpy(np.int64);O=p.o.to_numpy(float);H=p.h.to_numpy(float);L=p.l.to_numpy(float);C=p.c.to_numpy(float);Z=p.z.to_numpy(float);A=p.atr.to_numpy(float);Y=p.year.to_numpy(np.int64);D=p.day.to_numpy(np.int64)
    ...
    for params in itertools.product(CONFIRMS,TTLS,MODES,STOPS,HOLDS,PAUSES):
        rec=rec_from(params,run_sim(ts,O,H,L,C,Z,A,Y,D,*params,0,0,0,0,0),'none');coarse.append(rec);tested+=1
        if rec['positive_years']==5 and rec['agg']['N']>=150:perfect.append(rec)
    coarse.sort(key=lambda x:(x['positive_years'],x['score']),reverse=True);(OUT/'coarse_top200.json').write_text(json.dumps(coarse[:200],indent=2));print('coarse',tested,'perfect',len(perfect))
    ...
    for s in seeds:
        ...
        for ek,ba,bl,ta,td,en in EXITS:refined.append(rec_from(params,run_sim(ts,O,H,L,C,Z,A,Y,D,*params,ek,ba,bl,ta,td),en))
