...
def build_union_fixed(canon,child,a,b,months):
    ...
    # Persisted LAB015 canonical stream did not store actual exit_time.
    # For concurrency/risk only, use conservative parent +24h end; this can only overstate overlap.
    ...
    return dict(real_fills=len(z),fills_per_month=len(z)/months,canonical_fills=int((z.src=="CANON").sum()) if len(z) else 0,h4_fills=int((z.src=="H4_TWO_BAR").sum()) if len(z) else 0,cum_R=float(r.sum()) if len(r) else 0.0,mean_R=float(r.mean()) if len(r) else np.nan,pf=L18.pf(r),dd=L18.maxdd(r),max_concurrent=mc,risk_load_025_pct=mc*.25,risk_load_050_pct=mc*.50,equity_return_025_pct=er25,equity_dd_025_pct=ed25,equity_return_050_pct=er50,equity_dd_050_pct=ed50),z
...
if __name__=="__main__":
    ...
    if p.exists():
        ...
        txt += "\n## Risk-accounting note\n- LAB015 canonical artifact did not persist actual `exit_time`; union concurrency uses conservative `canonical event_time + 24h`. Therefore max concurrent risk is an upper-bound style audit and may overstate actual overlap. PnL, PF, EV and DD-R calculations are unaffected.\n"
