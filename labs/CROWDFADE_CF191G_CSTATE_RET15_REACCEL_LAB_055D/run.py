...
def period_pass(base,new,bpos,npos):
    return bool(new['SumR']>=base['SumR'] and new['EV']>base['EV'] and new['PF']>base['PF'] and new['MaxDD_R']<=base['MaxDD_R'] and new['R_DD']>base['R_DD'] and npos>=bpos and new['MaxConsecutiveLosses']<=base['MaxConsecutiveLosses'])
...
def sim_gate(p,gate):
    ...
    while k<len(dt5)-3:
        ...
        if conf:
            ...
            if gate:
                ret60=np.nan; eff60=np.nan; ret15_dir=np.nan
                if k>=12 and np.isfinite(A5[k]) and A5[k]>0:
                    p60=C5[k-12:k+1]
                    ret60=side*(p60[-1]-p60[0])/A5[k]
                    path=np.abs(np.diff(p60)).sum()
                    if path>0:
                        eff60=side*(p60[-1]-p60[0])/path
                    ret15_dir=side*(C5[k]-C5[k-3])/A5[k]
                if np.isfinite(ret60) and np.isfinite(eff60) and np.isfinite(ret15_dir) and ret60>1.0 and eff60<0.25 and ret15_dir<=0.0:
...
def main():
    ...
    for period,p in periods.items():
        ...
        rows += [dict(period=period,variant='CONTROL',**bm,positive_periods=bpos,periods_total=btot,skips=0),
                 dict(period=period,variant='SKIP_CSTATE_AND_RET15_DIR_LE_0',**nm,positive_periods=npos,periods_total=ntot,skips=nskip)]
        detail[period]={'control':bm,'new':nm,'skip_count':int(nskip),'control_periods':bper,'new_periods':nper,'PASS':passed}
    pd.DataFrame(rows).to_csv(OUT/'comparison.csv',index=False)
    result={'lab':'CROWDFADE_CF191G_CSTATE_RET15_REACCEL_LAB_055D','threshold_ret60_atr':1.0,'threshold_eff60':0.25,'threshold_ret15_dir':0.0,'detail':detail,'PASS_BOTH':overall,
    ...
    lines=['# LAB055D — C-STATE + RET15_DIR <= 0 SKIP','',pd.DataFrame(rows).to_markdown(index=False),'',json.dumps(result,indent=2,default=float)]
