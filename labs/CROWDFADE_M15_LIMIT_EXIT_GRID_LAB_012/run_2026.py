...
ROOT=Path('labs/CROWDFADE_M15_LIMIT_EXIT_GRID_LAB_012');DATA=ROOT/'stress_data';OUT=ROOT/'output';OUT.mkdir(parents=True,exist_ok=True)
ZTH=2.5;CONF=.25;CONF_TTL=3600;RETRACE=.60;PTTL=1200;COST=.5;MAXDAY=3;TF=900
CANDS=[('SL3_TP10_H24',3.,10.,24),('SL2.5_TP8_H24',2.5,8.,24),('SL3_TP12_H24',3.,12.,24),('SL4.5_TP10_H24',4.5,10.,24)]
def ex(z,o):
...
def prep():
 ...
 starts=np.arange(((ts[0]+899)//900)*900,ts[-1]-900+1,900,dtype=np.int64);idx=np.searchsorted(ts,starts);idx2=np.searchsorted(ts,starts+900)-1;ok=(idx<len(ts))&(idx2>=idx)&(idx2<len(ts));starts=starts[ok];idx2=idx2[ok];dt=starts+900;cl=C[idx2];fi=np.searchsorted(ft,dt,'left')-1;ai=np.searchsorted(av,dt,'right')-1;g=(fi>=0)&(ai>=0)&np.isfinite(z[np.maximum(fi,0)])&np.isfinite(atr[np.maximum(ai,0)])
...
def sim(arr,slv,tpv,hold):
 ...
 while k<len(dt)-2:
  ...
  ei=ps+int(ff[0]);sl=entry-side*slv*a;tp=entry+side*tpv*a;xe=min(len(ts)-1,np.searchsorted(ts,int(ts[ei])+hold*3600,'left'));xp=float(C[xe]);xi=xe;reason='TIME'
  ...
  R=side*(xp-entry)/(slv*a)-(COST/10000.)*entry/(slv*a);rows.append({'entry_ts':int(ts[ei]),'R':R,'reason':reason});dc+=1;last=entry;la=a;k=np.searchsorted(dt,int(ts[xi])+1)
 return pd.DataFrame(rows)
def main():
 arr=prep();out={}
 for name,sl,tp,h in CANDS:
  d=sim(arr,sl,tp,h);m=pd.to_datetime(d.entry_ts,unit='s',utc=True).dt.to_period('M').astype(str);out[name]={'config':{'sl':sl,'tp':tp,'hold':h},'all':stats(d.R),'exit_mix':d.reason.value_counts().to_dict(),'monthly':{x:stats(d.loc[m==x,'R']) for x in sorted(m.unique())}}
