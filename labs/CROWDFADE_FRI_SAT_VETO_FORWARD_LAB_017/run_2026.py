...
def prep():
    ...
    dt=dt[g]
    # Preserve LAB015 semantics: weekday of the M15 bar start, i.e. close timestamp minus 15 min.
    dow=pd.to_datetime(dt-900,unit='s',utc=True).dayofweek.to_numpy(np.int64)
    return ts,O,H,L,C,dt,cl[g],z[fi[g]],atr[ai[g]],dow
...
def stats(a,months_div=6.0):
    ...
    return {
        'N':int(len(a)),
        'trades_per_month':float(len(a)/months_div),
        'WR':float((a>0).mean()),
        'EV':float(a.mean()),
        'SumR':float(a.sum()),
...
def vetoed(dow,mode):
    if mode==1: return dow==4
    if mode==2: return dow==5
    if mode==3: return dow in (4,5)
    return False

def sim(arr,mode):
    ts,O,H,L,C,dt,cl,zd,ad,dow=arr
...
def main():
    ...
    for name,mode in MODES:
        d=sim(arr,mode)
        s=summarize(d)
        rows.append({'mode':name,'mode_id':mode,**s})
