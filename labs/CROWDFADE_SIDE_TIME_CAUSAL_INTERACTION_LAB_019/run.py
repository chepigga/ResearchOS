...
ROOT=Path('labs/CROWDFADE_SIDE_TIME_CAUSAL_INTERACTION_LAB_019')
...
def prep():
    ...
    q['hour']=q.time.dt.hour.astype('int64')
    return p,q

@njit(cache=True)
def is_vetoed(side,hour,mode):
    a=(hour>=4 and hour<=7)
    b=(hour>=18 and hour<=20)
    if mode==1:return side>0 and a
    if mode==2:return side<0 and a
    if mode==3:return side>0 and b
    if mode==4:return side<0 and b
    return False

@njit(cache=True)
def sim(ts,O,H,L,C,dt,CL,Z,A,Y,DAY,HOUR,mode):
    ...
    while k<len(dt)-3:
        ...
        if is_vetoed(side,HOUR[k],mode):k+=1;continue
        ...
        pe=np.searchsorted(ts,dt[ci]+PASSIVE_TTL_MIN*60,'right');ei=-1
        ...
        yi=Y[ci]-2021;si=0 if side>0 else 1
        if 0<=yi<5:
            sums[yi]+=R;cnt[yi]+=1
            annual_side[si,yi]+=R;annual_side_cnt[si,yi]+=1
        side_sums[si]+=R;side_cnt[si]+=1
        if R>0:wins+=1;pos+=R;side_pos[si]+=R;side_wins[si]+=1
        elif R<0:neg-=R;side_neg[si]-=R
...
def rec(name,mode,r):
    ...
    for si,sname in enumerate(['LONG','SHORT']):
        nn=int(sc[si])
        sides[sname]={
            'N':nn,'EV':float(ss[si]/nn) if nn else 0.,'SumR':float(ss[si]),
            'PF':float(sp[si]/sn[si]) if sn[si]>0 else 99.,'WR':float(sw[si]/nn) if nn else 0.,
            'annual':{str(2021+i):{
                'SumR':float(annual_side[si,i]),'N':int(annual_side_cnt[si,i]),
                'EV':float(annual_side[si,i]/annual_side_cnt[si,i]) if annual_side_cnt[si,i] else 0.
            } for i in range(5)}
        }
    return {'mode':name,'mode_id':int(mode),'N':int(n),'trades_per_month':float(n/60.0),
            'positive_years':int(np.sum(s>0)),'annual':annual,'by_side':sides,
            'exit_mix':{'SL':int(slh),'TP':int(tph),'TIME':int(timeh)},
            'agg':{'SumR':float(total),'EV':float(ev),'PF':float(pf),'MaxDD_R':float(dd),
                   'R_DD':float(total/max(dd,1e-9)),'WR':float(wr)}}
...
def main():
    ...
    arr=[p.ts.to_numpy(np.int64),p.o.to_numpy(float),p.h.to_numpy(float),p.l.to_numpy(float),p.c.to_numpy(float),
         ...
         q.yr.to_numpy(np.int64),q.daykey.to_numpy(np.int64),q.hour.to_numpy(np.int64)]
    sim(*[x[:1000] for x in arr],0)
    rows=[rec(name,mode,sim(*arr,mode)) for name,mode in MODES]
    b=rows[0]
    for x in rows:
        x['vs_baseline']={'dEV':float(x['agg']['EV']-b['agg']['EV']),
                          'dPF':float(x['agg']['PF']-b['agg']['PF']),
                          'dMaxDD_R':float(x['agg']['MaxDD_R']-b['agg']['MaxDD_R']),
                          'dSumR':float(x['agg']['SumR']-b['agg']['SumR']),
                          'dN':int(x['N']-b['N'])}
    out={'lab':'CROWDFADE_SIDE_TIME_CAUSAL_INTERACTION_LAB_019',
         'frozen_signal':{'ZLong':ZTH,'ZShort':ZTH},
         'frozen_execution':{'TF':'M15','ConfirmATR':CONF,'ConfirmTTL_min':CONF_TTL_MIN,'RetraceATR':RETRACE,
                             'LimitOnly':True,'LimitTTL_min':PASSIVE_TTL_MIN,'SL_ATR':SL,'TP_ATR':TP,'Hold_h':HOLD_H,
                             'Cost_bps':COST,'MaxTradesDay':MAXDAY,'InheritedAntiRepeatATR':1.0},
         'time_semantics':'UTC hour of original M15 signal bar start, matching LAB015/LAB018 buckets',
         'windows':{'A':'04:00-07:59 UTC','B':'18:00-20:59 UTC'},
         'design':'atomic side x time vetoes only; no combination tuning',
         'results':rows,
         'rank_robust':sorted(rows,key=lambda x:(x['positive_years'],min(v['SumR'] for v in x['annual'].values()),x['agg']['R_DD'],x['agg']['EV']),reverse=True)}
